...
# Simply load the checkpoints of a prior model
def load_checkpoints(config_path, checkpoint_path, gen, cpu=False):

    with open(config_path) as f: # Open the config
        config = yaml.safe_load(f) # Why do we need a yaml?
...
def make_animation(source_image, driving_video, generator, kp_detector, he_estimator, relative=True,
                   adapt_movement_scale=True, estimate_jacobian=True, cpu=False, free_view=False,
